...
<%
//************************************************************
// * <p>#File Name:   Merchant_CaptureVoid_List.jsp        </p>
// * <p>#Description:                  </p>
...
// * @since       SPEC version
// * @version 
// * 2020/07/02 彥仲 202007070141-00 修正chrome瀏覽器無法請款取消的狀況
// ************************************************************/
  try {
...
//   <!--
  ...
  function toSubmit() {
        ...
        if(objs[i].type=="checkbox"){
           if(objs[i].checked==true){
               if (objs[i].value.length>0) {
                 var Data = objs[i].value;
                 var arrayData = Data.split("||");
                 if (arrayData.length == 2) {
                   if(count==0){
                     ROWID = ROWID + arrayData[0];
                     TERMINALID = TERMINALID + arrayData[1];
                   } else{
                     ROWID = ROWID  + "," + arrayData[0];
                     TERMINALID = TERMINALID  + "," + arrayData[1];
                   }
     ...
     if (ROWID.length == 0 || TERMINALID.length == 0 ) {
        alert("未選取請款取消資料");
        return void(0);
     } else {
        document.form.InputROWID.value=ROWID;
        document.form.InputTERMINALID.value=TERMINALID;
...
<input type="hidden" name="InputROWID" id="InputROWID" value="">
<input type="hidden" name="InputTERMINALID" id="InputTERMINALID" value="">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="left" valign="top" width="12"><img src="images/sub_title_icon.jpg"></td>
      <td align="left" valign="middle"><b><font color="#004E87" size="3">線上請款取消作業</font></b></td>
  </tr>
</table>
...
<hr style="height:1px">
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="1" bgcolor="#D8D8D8">
    ...
    for (int c=0; c<arrayList.size(); ++c) {
      ...
      String CaptureFlag = hashData.get("CAPTUREFLAG").toString();
      if (TransCode.equalsIgnoreCase("00") && CaptureFlag.equalsIgnoreCase("0")) {  // 購貨
        if (CaptureAmt.length()>0) {
          CancelAmt = String.valueOf(Double.parseDouble(CancelAmt)+Double.parseDouble(CaptureAmt));
        }
      }
      if (TransCode.equalsIgnoreCase("01") && CaptureFlag.equalsIgnoreCase("0")) {  // 退貨
        FontColor = "#FF0000";
        if (CaptureAmt.length()>0) {
          CancelAmt = String.valueOf(Double.parseDouble(CancelAmt)+Double.parseDouble(CaptureAmt));
        }
...
<tr align='center' bgcolor='#ffffff'  height="25">
  ...
  <td align='left'   rowspan="2"><%if ((TransCode.equalsIgnoreCase("00") || TransCode.equalsIgnoreCase("01")) && CaptureFlag.equalsIgnoreCase("0") ) {%><input type='checkbox' id='Checkbox"<%=String.valueOf(count)%>"' value='<%=hashData.get("RROWID").toString()%>||<%=hashData.get("TERMINALID").toString()%>'><%}%></td>
  ...
  if (CaptureAmt.length()>0) {
    ...
    <td align='center'  rowspan="2"><font color="<%=FontColor%>"><%
    if (CaptureFlag.equalsIgnoreCase("0")) CaptureFlag += "-未處理";
    if (CaptureFlag.equalsIgnoreCase("1")) CaptureFlag += "-處理中";
    if (CaptureFlag.equalsIgnoreCase("2")) CaptureFlag += "-已取消";
    if (CaptureFlag.equalsIgnoreCase("3")) CaptureFlag += "-已處理";
    out.write(CaptureFlag);
    %></font></td>
  </tr>
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="1" bgcolor="#D8D8D8">
    ...
    <tr bgcolor='#ffffff' height="25">
      <td align='left' width='15%'>可取消請款金額</td>
...
<% if (VoidFlag.equalsIgnoreCase("true") ){ %>
    ...
    <tr>
      <td>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td height="30" align="left"><input type='button' value='確定取消' name='btnSubmit' id='btnSubmit' onclick="toSubmit();">
