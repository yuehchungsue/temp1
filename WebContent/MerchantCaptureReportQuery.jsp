...
function toPrint(PrintType) {
  if(checkdata()){
    ...
    // 請款交易統計報表 開啟列印的網址加入傳送 TransType的值 by Jimmy Kang 20150803
    var url = "<%=request.getContextPath()%>/MerchantCaptureReportCtl?Action=Query&<%=MerchantMenuCtl.MENU_FORWARD_KEY%>=<%=MenuKey%>&Start_TransDate="+document.all.form.Start_TransDate.value+"&End_TransDate="+document.all.form.End_TransDate.value+"&TransCode="+document.all.form.TransCode.value+"&RrportItem="+document.all.form.RrportItem.value +"&PrintType="+PrintType+"&FeedBackCode="+document.all.form.FeedBackCode.value+"&subMid="+document.all.form.subMid.value+"&TransType="+document.all.form.TransType.value ;
...
<body bgcolor="#ffffff">
<form method="post" name="form" id="form" action="./MerchantCaptureReportCtl">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
   ...
   <td align="left" valign="middle"><b><font color="#004E87" size="3">請款交易統計報表</font></b></td>
  </tr>
 </table>
 ...
 <table width="100%" border="0" cellspacing="1" cellpadding="1" bgcolor='#D8D8D8'>
  ...
  <tr align='center' height="25">
   ...
   <td width='75%' align='left'  bgcolor='#FFFFFF'><select name='TransCode' id='TransCode'><option value='CAPTURE'>請款交易</option><option value='CANCEL'>請款取消</option></select></td>
  </tr>
  
  <!-- 請款交易統計報表 Jimmy Kang 20150803 查詢條件 新增  TransType 下拉選單  -- 新增開始 -- -->
  <tr align='center' height="25">
  ...
  <!-- 請款交易統計報表 Jimmy Kang 20150803 查詢條件 新增  TransType 下拉選單  -- 新增結束 -- -->
  

  <tr align='center' height="25">
   <td width='25%' align='right' bgcolor='#F4F4FF'>請款結果</td>
   <td width='75%' align='left'  bgcolor='#FFFFFF'><select name='FeedBackCode' id='FeedBackCode'><option value='ALL'>全部</option><option value='APPROVE'>核准</option><option value='REJECT'>拒絕</option><option value='PROCESS'>處理中</option></select></td>
  </tr>
