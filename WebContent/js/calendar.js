...
function datepopup(x){
  x.value= "";
  var calender = new calendar2(x);
  calender.year_scroll = true;
  calender.time_comp = false;
  calender.popup()
}
...
function cal_prs_date2 (str_date) {  
	...
	if (!RE_NUM.exec(arr_date[0])) {
		return today;
		//return alert ("Invalid year value: '" + arr_date[0] + "'.\nAllowed values are unsigned integers.");
	}
	if (!arr_date[1]) {
		return today;
	...
	if (!RE_NUM.exec(arr_date[1])) {
		return today;
		 //return alert ("Invalid month value: '" + arr_date[1] + "'.\nAllowed values are unsigned integers.");
		}
	if (!arr_date[2]){
		return today;
	...
	if (!RE_NUM.exec(arr_date[2])) {
		return today;
		 //return alert ("Invalid day of month value: '" + arr_date[2] + "'.\nAllowed values are unsigned integers.");
	}
	...
	if (arr_date[0] < 100) {
		 arr_date[0] = Number(arr_date[2]) + (arr_date[2] < NUM_CENTYEAR ? 2000 : 1900);
		}
